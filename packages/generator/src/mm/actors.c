...
static int canSpawnSoul(PlayState *play, s16 actorId, u16 variable)
{
    ...
    case ACTOR_EN_NIW:
    case ACTOR_EN_NWC:
        return opt(comboHasSoulMm(GI_MM_SOUL_ANIMAL_CUCCO));
    case ACTOR_EN_COW:
        return opt(comboHasSoulMm(GI_MM_SOUL_ANIMAL_COW));
    case ACTOR_EN_DG:
...
static int canSpawnActor(PlayState *play, s16 actorId, u16 variable)
{
    ...
    case ACTOR_EN_SYATEKI_OKUTA:
        if (play->sceneId == SCE_MM_SHOOTING_GALLERY && (gSave.day > 3 || gSave.day < 1))
            ...
            return 1;
    case ACTOR_DOOR_ANA:
        return !(Config_Flag(CFG_MM_SHOVEL) && !SaveFlag_Get(SAVE_FLAG_SHOVEL_MM));
    default:
        return 1;
    }
}
...
void Actor_DrawFaroresWindPointer(PlayState *play)
{
    ...
    if (params && gSaveContext.respawn[RESPAWN_MODE_HUMAN].entrance != ENTR_FW_CROSS)
    {
        ...
        else if (temp > 0)
        {
            ...
            if (alpha < 0)
            {
                gCustomSave.fw[comboMmFwAge()].data = 0;
                gSaveContext.respawn[RESPAWN_MODE_HUMAN].data = 0;
                alpha = 0;
            }
            else
...
ActorProfile *Actor_LoadOverlayCustom(ActorContext *actorCtx, s16 index)
{
    ...
    if (overlayEntry->count == 0 && overlayEntry->data)
    {
        ...
        {
            case ACTOR_EN_HORSE:
                EponaAge_OnHorseOverlayLoaded(overlayEntry->data);
                break;
        }
    }
...
#define PLAYER_AGE_PROPERTIES_TABLE \
    ((PlayerAgeProperties*)OverlayAddr(0x8085BA38))

#define PLAYER_ANIMATION_HEADERS \
    ((PlayerAnimationHeader**)OverlayAddr(0x8085BE84))

#define PLAYER_ANIMTYPE_COUNT 6

#define PLAYER_DOOR_A_BASE 0x36
#define PLAYER_DOOR_B_BASE 0x42

static PlayerAnimationHeader*
    sChildDoorAAnimations[PLAYER_ANIMTYPE_COUNT];

static PlayerAnimationHeader*
    sChildDoorBAnimations[PLAYER_ANIMTYPE_COUNT];

static s32 sPlayerAgeOverlayDefaultsValid;

static void PlayerAge_CaptureOverlayDefaults(void)
{
    PlayerAnimationHeader** table = PLAYER_ANIMATION_HEADERS;

    memcpy(
        sChildDoorAAnimations,
        &table[PLAYER_DOOR_A_BASE],
        sizeof(sChildDoorAAnimations)
    );

    memcpy(
        sChildDoorBAnimations,
        &table[PLAYER_DOOR_B_BASE],
        sizeof(sChildDoorBAnimations)
    );

    sPlayerAgeOverlayDefaultsValid = 1;
}

void PlayerAge_ApplyOverlayData(s32 adult)
{
    PlayerAgeProperties* humanProperties =
        &PLAYER_AGE_PROPERTIES_TABLE[MM_PLAYER_FORM_HUMAN];

    PlayerAnimationHeader** table =
        PLAYER_ANIMATION_HEADERS;
    ...
    LoadFile(
        ...
        sizeof(*humanProperties)
    );

    if (adult)
    {
        for (i = 0; i < PLAYER_ANIMTYPE_COUNT; i++)
        {
            table[PLAYER_DOOR_A_BASE + i] =
                (PlayerAnimationHeader*)0x0400E3C8;

            table[PLAYER_DOOR_B_BASE + i] =
                (PlayerAnimationHeader*)0x0400E3D0;
        }
    }
    else if (sPlayerAgeOverlayDefaultsValid)
    {
        memcpy(
            &table[PLAYER_DOOR_A_BASE],
            sChildDoorAAnimations,
...
void KaleidoManager_LoadPlayerOvl(void* ovl)
{
    ...
    PlayerAge_CaptureOverlayDefaults();
    PlayerAge_ApplyOverlayData(comboIsLinkAdult());
}
