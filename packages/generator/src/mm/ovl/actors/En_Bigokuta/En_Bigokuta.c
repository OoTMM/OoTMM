...
void EnBigokuta_Init(Actor *thisx, PlayState *play)
{
    ...
    SkelAnime_InitFlex(play, &this->skelAnime, (FlexSkeletonHeader *)&gBigOctoSkel, (AnimationHeader *)&gBigOctoIdleAnim, this->jointTable, this->morphTable,
                       BIGOKUTA_LIMB_MAX);
...
s32 EnBigokuta_ValidatePictograph(PlayState *play, Actor *thisx)
{
    return Snap_ValidatePictograph(play, thisx, 3 /* PICTO_VALID_BIG_OCTO */, &thisx->focus.pos, &thisx->shape.rot, 280.0f,
                                   1800.0f, -1);
}
...
void EnBigokuta_SetupIdle(EnBigokuta *this)
{
    Animation_Change(&this->skelAnime, (AnimationHeader *)&gBigOctoIdleAnim, 0.5f, 0.0f, 0.0f, ANIMMODE_LOOP_INTERP, -3.0f);
...
void EnBigokuta_SetupRise(EnBigokuta *this, PlayState *play)
{
    ...
    Animation_PlayOnce(&this->skelAnime, (AnimationHeader *)&gBigOctoRiseOutOfWaterAnim);
...
void EnBigokuta_SetupIdleAboveWater(EnBigokuta *this)
{
    Animation_MorphToLoop(&this->skelAnime, (AnimationHeader *)&gBigOctoIdleAnim, -5.0f);
...
void EnBigokuta_IdleAboveWater(EnBigokuta *this, PlayState *play)
{
    ...
    if ((this->picto.actor.xzDistToPlayer > 400.0f) || (this->picto.actor.yDistanceFromLink > 200.0f))
    {
...
void EnBigokuta_SetupSuckInPlayer(EnBigokuta *this, PlayState *play)
{
    ...
    Animation_Change(&this->skelAnime, (AnimationHeader *)&gBigOctoIdleAnim, 1.0f, 12.0f, 12.0f, ANIMMODE_ONCE, -3.0f);
    CutsceneManager_Queue(this->picto.actor.csId);
...
void EnBigokuta_SetupDeathCutscene(EnBigokuta *this)
{
    ...
    if (Config_Flag(CFG_MM_SKIP_CUTSCENES))
    {
        if (EN_BIGOKUTA_GET_SWITCH_FLAG(&this->picto.actor) != 0xFF)
        {
            Flags_SetSwitch(gPlay, EN_BIGOKUTA_GET_SWITCH_FLAG(&this->picto.actor));
        }
...
void EnBigokuta_PlayDeathCutscene(EnBigokuta *this, PlayState *play)
{
    ...
    else if (CutsceneManager_IsNext(this->csId))
    {
        ...
        if (!MM_CHECK_EVENT_INF(EVENTINF_41) && !MM_CHECK_EVENT_INF(EVENTINF_35))
        {
        ...
        else
        {
            ...
            player->stateFlags1 |= PLAYER_STATE1_MM_20;
        }
...
void EnBigokuta_SetupDeathEffects(EnBigokuta *this)
{
    Animation_MorphToPlayOnce(&this->skelAnime, (AnimationHeader *)&gBigOctoDeathAnim, -5.0f);
...
void EnBigokuta_CheckOneHitKill(EnBigokuta *this, PlayState *play)
{
    if ((this->bodyCollider.base.acFlags & AC_ON) &&
        ((this->bodyCollider.base.acFlags & AC_HIT) ||
         ((play->sceneId == SCE_MM_SOUTHERN_SWAMP || play->sceneId == SCE_MM_SOUTHERN_SWAMP_CLEAR) &&
          EnBigokuta_IsNearSwampBoat(this, play))))
    {
        ...
        {
            if (this->bodyCollider.elem.acHitElem->atDmgInfo.dmgFlags & 0x1000)
            { // Ice Arrow
            ...
            else if (this->bodyCollider.elem.acHitElem->atDmgInfo.dmgFlags & 0x2000)
            { // Light Arrow
...
OVL_INFO_ACTOR(ACTOR_EN_BIGOKUTA, En_Bigokuta_Profile);
