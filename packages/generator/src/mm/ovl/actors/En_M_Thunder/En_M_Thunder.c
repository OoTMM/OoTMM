#include "En_M_Thunder.h"
#include <actors/Eff_Dust/Eff_Dust.h>
...
void EnMThunder_Update(EnMThunder* this, PlayState* play);
void EnMThunder_Draw(EnMThunder* this, PlayState* play2);

void EnMThunder_UnkType_Update(EnMThunder* this, PlayState* play);
...
void EnMThunder_Init(EnMThunder* this, PlayState* play) {
    ...
    if (player->stateFlags2 & PLAYER_STATE2_MM_20000) {
        if (!gSave.info.playerData.isMagicAcquired || (gSaveContext.magicState != MAGIC_STATE_IDLE) ||
            ((ENMTHUNDER_GET_MAGIC_COST(&this->actor) != 0) &&
             !Magic_RequestChange(play, ENMTHUNDER_GET_MAGIC_COST(&this->actor), MAGIC_CONSUME_NOW))) {
        ...
        player->stateFlags2 &= ~PLAYER_STATE2_MM_20000;
        this->isCharging = false;

        if (MM_GET_EVENT_WEEK(EV_MM_WEEK_SPIN_UPGRADE)) {
            player->unk_B08 = 1.0f;
...
void EnMThunder_Spin_AttackNoMagic(EnMThunder* this, PlayState* play) {
    ...
    if (!(player->stateFlags1 & PLAYER_STATE1_MM_1000)) {
        Actor_Kill(&this->actor);
    }
}
...
void EnMThunder_Charge(EnMThunder* this, PlayState* play) {
    ...
    if (!this->isCharging && (player->unk_B08 >= 0.1f)) {
        if ((gSaveContext.magicState != MAGIC_STATE_IDLE) ||
            ((ENMTHUNDER_GET_MAGIC_COST(&this->actor) != 0) &&
             !Magic_RequestChange(play, ENMTHUNDER_GET_MAGIC_COST(&this->actor), MAGIC_CONSUME_WAIT_PREVIEW))) {
...
void EnMThunder_Spin_Attack(EnMThunder* this, PlayState* play) {
    ...
    // Do not cull the spin attack when collecting an item
...
void EnMThunder_Draw(EnMThunder* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    f32 scale;
    s32 y2Scroll;
...
OVL_INFO_ACTOR(ACTOR_EN_M_THUNDER, EnMThunder_InitVars);
