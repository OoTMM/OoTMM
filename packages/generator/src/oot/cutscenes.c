...
static void OnePointCutscene_Sfx(s32 *shouldCsPlay, s32 playSfx)
{
    *shouldCsPlay = !Config_Flag(CFG_OOT_SKIP_CUTSCENES);
...
static s32 OnePointCutscene_ShouldCutscenePlay(PlayState *play, Actor *actor)
{
    ...
    if (!Config_Flag(CFG_OOT_CUTSCENE_GLITCH_AID) && !Config_Flag(CFG_OOT_SKIP_CUTSCENES))
        return true;
