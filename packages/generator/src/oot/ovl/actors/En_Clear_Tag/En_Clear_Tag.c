...
static TexturePtr sWaterSplashTextures[] = {
    (TexturePtr) 0x04025A90, /* gEffWaterSplash1Tex */
    (TexturePtr) 0x04025F90, /* gEffWaterSplash2Tex */
    (TexturePtr) 0x04026490, /* gEffWaterSplash3Tex */
    (TexturePtr) 0x04026990, /* gEffWaterSplash4Tex */
    (TexturePtr) 0x04026E90, /* gEffWaterSplash5Tex */
    (TexturePtr) 0x04027390, /* gEffWaterSplash6Tex */
    (TexturePtr) 0x04027890, /* gEffWaterSplash7Tex */
    (TexturePtr) 0x04027D90, /* gEffWaterSplash8Tex */
    NULL,
    NULL,
    NULL,
};
...
void EnClearTag_CreateLightRayEffect(EnClearTag* this, Vec3f* pos, Vec3f* velocity, Vec3f* accel, f32 scale,
    ...
    for (i = 0; i < ARRAY_COUNT(this->effect) - 1; i++, effect++) {
        if (effect->type == CLEAR_TAG_EFFECT_AVAILABLE) {
            ...
            effect->rotationY = Math_Atan2F(effect->velocity.z, effect->velocity.x);
            effect->rotationX = -Math_Atan2F(sqrtf(SQXZ(effect->velocity)), effect->velocity.y);
        ...
        if (thisx->params != CLEAR_TAG_SPLASH) {
            ...
            if ((thisx->params == CLEAR_TAG_SMALL_LIGHT_RAYS) || (thisx->params == CLEAR_TAG_LARGE_LIGHT_RAYS)) {
                for (i = 0; i < 54; i++) {
                    ...
                    Matrix_RotateXF(Rand_ZeroFloat(M_PIf * 2), MTXMODE_APPLY);
            ...
            Actor_UpdateBgCheckInfo(play, &this->actor, 50.0f, 30.0f, 100.0f, UPDBGCHECKINFO_FLAG_FLOOR);
            ...
            for (i = 0; i < 44; i++) {
                ...
                Matrix_RotateXF(Rand_ZeroFloat(2 * M_PIf), MTXMODE_APPLY);
                Matrix_MultVecZ(lightRayMaxScale, &velocity);
                accel.x = velocity.x * -0.03f;
                accel.y = velocity.y * -0.03f;
                accel.z = velocity.z * -0.03f;
...
void EnClearTag_UpdateCamera(EnClearTag* this, PlayState* play) {
    ...
    switch (this->cameraState) {
        ...
        case 1:
            ...
            this->cameraState = 2;
            Audio_PlaySfx_AtPosWithReverb(&gSfxDefaultPos, 0x6843, 0x20); /* NA_SE_VO_NAVY_CALL */
            // fallthrough
        case 2:
            if (player->actor.world.pos.z > 0.0f) {
                player->actor.world.pos.z = 290.0f;
...
void EnClearTag_DrawEffects(Actor* thisx, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->effect); i++, effect++) {
        if (effect->type == CLEAR_TAG_EFFECT_DEBRIS) {
            ...
            if (!isMaterialApplied) {
                isMaterialApplied++;
                gSPDisplayList(POLY_OPA_DISP++, CUSTOM_OBJECT_CLEAR_TAG_0);
            }
            ...
            Matrix_RotateXF(effect->rotationX, MTXMODE_APPLY);
            gSPMatrix(POLY_OPA_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_OPA_DISP++, CUSTOM_OBJECT_CLEAR_TAG_1);
        }
    }
    ...
    if (this->actor.floorPoly != NULL) {
        for (i = 0; i < ARRAY_COUNT(this->effect); i++, effect++) {
            if (effect->type == CLEAR_TAG_EFFECT_SHOCKWAVE) {
                ...
                func_80038A28(this->actor.floorPoly, effect->pos.x, effect->pos.y, effect->pos.z, &mtxF);
                Matrix_Put(&mtxF);
                Matrix_Scale(effect->scale, 1.0f, effect->scale, MTXMODE_APPLY);
                gSPMatrix(POLY_XLU_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
                gSPDisplayList(POLY_XLU_DISP++, 0x0401a550); /* gEffShockwaveDL */
            }
        }
    }
    ...
    if (this->actor.floorPoly != NULL) {
        for (i = 0; i < ARRAY_COUNT(this->effect); i++, effect++) {
            if (effect->type == CLEAR_TAG_EFFECT_FLASH) {
                ...
                func_80038A28(this->actor.floorPoly, effect->pos.x, this->actor.floorHeight, effect->pos.z, &mtxF);
                Matrix_Put(&mtxF);
                Matrix_Scale(effect->scale * 3.0f, 1.0f, effect->scale * 3.0f, MTXMODE_APPLY);
                gSPMatrix(POLY_XLU_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
                gSPDisplayList(POLY_XLU_DISP++, CUSTOM_OBJECT_CLEAR_TAG_5);
            }
        }
    }
    ...
    for (i = 0; i < ARRAY_COUNT(this->effect); i++, effect++) {
        if ((effect->type == CLEAR_TAG_EFFECT_SMOKE) || (effect->type == CLEAR_TAG_EFFECT_ISOLATED_SMOKE)) {
            ...
            if (!isMaterialApplied) {
                gSPDisplayList(POLY_XLU_DISP++, CUSTOM_OBJECT_CLEAR_TAG_2);
                isMaterialApplied++;
            }
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, CUSTOM_OBJECT_CLEAR_TAG_3);
        }
    }
    ...
    for (i = 0; i < ARRAY_COUNT(this->effect); i++, effect++) {
        if (effect->type == CLEAR_TAG_EFFECT_FLASH) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, CUSTOM_OBJECT_CLEAR_TAG_4);
        }
    }
        ...
        if (effect->type == CLEAR_TAG_EFFECT_LIGHT_RAYS) {
            ...
            if (!isMaterialApplied) {
                ...
                gDPSetEnvColor(POLY_XLU_DISP++, (u8)effect->envColor.r, (u8)effect->envColor.g, (u8)effect->envColor.b,
                               0);
                gSPDisplayList(POLY_XLU_DISP++, CUSTOM_OBJECT_CLEAR_TAG_6);
                isMaterialApplied++;
            }
            ...
            Matrix_RotateXF(M_PIf / 2, MTXMODE_APPLY);
            gSPMatrix(POLY_XLU_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, CUSTOM_OBJECT_CLEAR_TAG_7);
        }
    }
    ...
    for (i = 0; i < ARRAY_COUNT(this->effect); i++, effect++) {
        if (effect->type == CLEAR_TAG_EFFECT_SPLASH) {
            ...
            gSPSegment(POLY_XLU_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(sWaterSplashTextures[effect->actionTimer]));
            ...
            for (j = 0; j < 16; j++) {
                ...
                if (WaterBox_GetSurface1(play, &play->colCtx, effect->pos.x + vec.x, effect->pos.z + vec.z, &ySurface,
                    ...
                    if ((effect->pos.y - ySurface) < 200.0f) {
                        ...
                        Matrix_Scale(effect->scale, effect->scale, effect->scale, MTXMODE_APPLY);
                        gSPMatrix(POLY_XLU_DISP++, Matrix_Finalize(gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
                        gSPDisplayList(POLY_XLU_DISP++, 0x04028290); /* gEffWaterSplashDL */
                    }
                }
            }
        }
...
ActorProfile En_Clear_Tag_InitVars = {
    /**/ ACTOR_EN_CLEAR_TAG,
    /**/ ACTORCAT_ITEMACTION,
    /**/ FLAGS,
    /**/ CUSTOM_OBJECT_ID_CLEAR_TAG,
    /**/ sizeof(EnClearTag),
    /**/ EnClearTag_Init,
    /**/ EnClearTag_Destroy,
    /**/ EnClearTag_Update,
...
OVL_INFO_ACTOR(ACTOR_EN_CLEAR_TAG, En_Clear_Tag_InitVars);
