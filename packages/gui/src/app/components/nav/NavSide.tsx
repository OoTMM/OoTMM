...
function NavSideTab({ tab, isActive, onClick }: { tab: VNode<ComponentProps<typeof Tab>>, isActive: boolean, onClick: () => void }) {
  return (
    <a
  ...
  return (
    <div className="flex flex-row h-full w-full min-h-screen">
      <nav className="w-[200px] flex flex-col dark:bg-gray-950 border-r dark:border-gray-800">
        {tabs.map((tab, i) => <NavSideTab key={i} tab={tab} isActive={i === activeTab} onClick={() => setActiveTab(i)} />)}
      </nav>
