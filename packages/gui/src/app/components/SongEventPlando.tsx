import type { SongEventSongs } from '@ootmm/core';

import { SONG_EVENT_SONGS, SONG_EVENT_LOCATIONS_OOT, SONG_EVENT_LOCATIONS_MM } from '@ootmm/core';
...
type PlandoSongEventSong = SongEventSongs | 'random';
...
function songEventSongName(song: SongEventSongs): string {
  const name = SONG_EVENT_SONG_NAMES[song];
  if (!name) {
    return `Unknown Song (${song})`;
  }
  return name;
}
...
function intersectSongPools(pools: string[][]): string[] {
  if (pools.length === 0) {
    return [];
  }
...
function getAllowedSongPoolForGame(settings: Settings, game: Game): SongEventSongs[] {
  if (game === 'oot') {
    return unique([
      'ZELDAS_LULLABY',
      'EPONAS',
      ...
      'NOCTURNE',
      'PRELUDE',
      ...(settings.songHealingOot ? ['HEALING'] : []),
      ...(settings.songSoaringOot ? ['SOARING'] : []),
      ...(settings.songAwakeningOot ? ['SONATA'] : []),
      ...(settings.songGoronOot ? ['GORON_LULLABY'] : []),
      ...(
      ...
      ...(settings.songZoraOot ? ['NEW_WAVE'] : []),
      ...(settings.elegyOot ? ['ELEGY'] : []),
      ...(settings.songOrderOot ? ['OATH'] : []),
    ]);
  }

  return unique<SongEventSongs>([
    'TIME',
    'HEALING',
    'EPONAS',
    'SOARING',
    ...
    ...(settings.progressiveGoronLullabyMm === 'progressive'
      ? ['GORON_LULLABY_INTRO']
      : []),
      ...(settings.songZeldaLullabyMm ? ['ZELDAS_LULLABY'] : []),
      ...(settings.songSariasMm ? ['SARIAS'] : []),
      ...(settings.songSunMm ? ['SUNS'] : []),
      ...(settings.songMinuetMm ? ['MINUET'] : []),
      ...(settings.songBoleroMm ? ['BOLERO'] : []),
      ...(settings.songSerenadeMm ? ['SERENADE'] : []),
      ...(settings.songRequiemMm ? ['REQUIEM'] : []),
      ...(settings.songNocturneMm ? ['NOCTURNE'] : []),
      ...(settings.songPreludeMm ? ['PRELUDE'] : []),
    ]);
  }
  ...
  function isSongAllowedForGame(settings: Settings, song: SongEventSongs, game: Game) {
    ...
    const baseEventOptions = useMemo(() => {
      ...
      if (settings.songEventsShuffleOot) {
        ...
        const eventOptions = useMemo(() => {
          return baseEventOptions.filter(option => {
            ...
            if (selectedSong !== 'random') {
              return isSongAllowedForGame(settings, selectedSong, option.game);
            }
        ...
        const placeSongEvent = useCallback(() => {
          ...
          } else if (!allowedSongs.includes(song)) {
            return;
          }
