...
import type { ItemPools, LogicPassSolverState, SolverState } from './types';

import { Monitor, sample, shuffle, countMapAdd, countMapArray, countMapCombine, countMapRemove, ItemHelpers, ItemGroups, Items, makePlayerItem } from '@ootmm/core';
...
export class ItemPlacer {
  ...
  constructor(
    ...
    this.locations = input.worlds.map((x, i) => [...x.locations].map(l => makeLocation(l, i))).flat();
  ...
  init() {
    this.state.startingItems = new Map(this.input.startingItems);
    this.makeItemPools();
  ...
  private placeNamedTriforceWorld(worldId: number | 'all') {
    ...
    for (;;) {
      ...
      const region = this.input.worlds[locWorld].regions[locationData(l).id];
      if (region !== 'NONE') {
    ...
    for (const reward of rewards) {
      ...
      if (this.input.settings.dungeonRewardShuffle === 'dungeonsLimited') {
        ...
        for (const c of candidates) {
          const world = this.input.worlds[c.player];
