...
void Health_InitMeter(PlayState* play) {
    ...
    interfaceCtx->unk_226 = gOotSave.info.playerData.health;
...
s32 func_80078E18(PlayState* play) {
    gOotSave.info.playerData.health = play->interfaceCtx.unk_226;
    return 1;
}
...
s32 func_80078E34(PlayState* play) {
    ...
    if (interfaceCtx->unk_226 >= gOotSave.info.playerData.health) {
        interfaceCtx->unk_226 = gOotSave.info.playerData.health;
        return 1;
    }
...
s32 func_80078E84(PlayState* play) {
    ...
    if (interfaceCtx->unk_228 != 0) {
        ...
        if (interfaceCtx->unk_226 <= 0) {
            ...
            play->damagePlayer(play, -(gOotSave.info.playerData.health + 1));
            return 1;
        }
    }
    return 0;
...
void Health_DrawMeter(PlayState* play) {
    ...
    s32 curHeartFraction = gOotSave.info.playerData.health % 0x10;
    s16 totalHeartCount = gOotSave.info.playerData.healthCapacity / 0x10;
    s16 fullHeartCount = gOotSave.info.playerData.health / 0x10;
    s32 pad2;
    f32 beatingHeartPulsingSize = interfaceCtx->beatingHeartOscillator * 0.1f;
    s32 curCombineModeSet = 0;
    u8* curBgImgLoaded = NULL;
    s32 ddHeartCountMinusOne = gOotSave.info.inventory.defenseHearts - 1;

    OPEN_DISPS(gfxCtx, "../z_lifemeter.c", 353);

    if (!(gOotSave.info.playerData.health % 0x10)) {
        fullHeartCount--;
    }
...
u32 Health_IsCritical(void) {
    ...
    if (gOotSave.info.playerData.healthCapacity <= 0x50) {
        criticalHealth = 0x10;
    } else if (gOotSave.info.playerData.healthCapacity <= 0xA0) {
        criticalHealth = 0x18;
    } else if (gOotSave.info.playerData.healthCapacity <= 0xF0) {
        criticalHealth = 0x20;
    } else {
        criticalHealth = 0x2C;
    }

    if ((criticalHealth >= gOotSave.info.playerData.health) &&
