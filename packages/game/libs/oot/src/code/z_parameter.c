            ...
            if ((Player_GetEnvironmentalHazard(play) >= PLAYER_ENV_HAZARD_UNDERWATER_FLOOR) &&
                ...
                if (interfaceCtx->restrictions.bButton == 0) {
                    if ((gOotSave.info.equips.buttonItems[0] == ITEM_SLINGSHOT) ||
                        ...
                        (gOotSave.info.equips.buttonItems[0] == ITEM_NONE)) {
                        if ((gOotSave.info.equips.buttonItems[0] != ITEM_NONE) ||
                            (gOotSave.info.infTable[INFTABLE_INDEX_1DX] == 0)) {
                            gOotSave.info.equips.buttonItems[0] = gSaveContext.buttonStatus[0];
                            sp28 = true;
...
void Inventory_SwapAgeEquipment(void) {
    ...
    shieldEquipValue = gOotSave.info.equips.equipment & gEquipMasks[EQUIP_TYPE_SHIELD];
    if (shieldEquipValue) {
...
u8 Item_Give(PlayState* play, u8 item) {
    ...
    if ((item >= ITEM_MEDALLION_FOREST) && (item <= ITEM_MEDALLION_LIGHT)) {
        ...
        if (temp != ITEM_NONE) {
            for (i = 1; i < 4; i++) {
                if (gOotSave.info.equips.buttonItems[i] == temp) {
