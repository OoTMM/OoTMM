...
void EnTorch2_Init(Actor* thisx, PlayState* play2) {
    ...
    this->actor.colChkInfo.health = gOotSave.info.playerData.healthCapacity >> 3;
...
s32 EnTorch2_SwingSword(PlayState* play, Input* input, Player* this) {
    ...
    if (gOotSave.info.playerData.health < 0x50) {
        attackDelay = 15;
        noAttackChance += 0.3f;
    }
    if (sAlpha != 255) {
    ...
    switch (sActionState) {
        ...
        case ENTORCH2_ATTACK:
            ...
            if ((this->meleeWeaponQuads[0].base.acFlags & AC_BOUNCED) ||
                 ...
                 */
                if ((gOotSave.info.playerData.health < 0x50) && (sCounterState != 0)) {
                    sCounterState = 0;
                    sStaggerTimer = 50;
                }
            }
