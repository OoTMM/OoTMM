    ...
    switch (this->actor.params) {
        ...
        case 5:
            ...
            if (gSaveContext.cutsceneTrigger != 0) {
                if (gOotSave.entranceIndex == ENTR_INSIDE_GANONS_CASTLE_2) {
                    this->effects[0].size = 0.1f;
                }
                if (gOotSave.entranceIndex == ENTR_INSIDE_GANONS_CASTLE_3) {
                    this->effects[1].size = 0.1f;
                }
                if (gOotSave.entranceIndex == ENTR_INSIDE_GANONS_CASTLE_4) {
                    this->effects[2].size = 0.1f;
                }
                if (gOotSave.entranceIndex == ENTR_INSIDE_GANONS_CASTLE_5) {
                    this->effects[3].size = 0.1f;
                }
                if (gOotSave.entranceIndex == ENTR_INSIDE_GANONS_CASTLE_6) {
                    this->effects[4].size = 0.1f;
                }
                if (gOotSave.entranceIndex == ENTR_INSIDE_GANONS_CASTLE_7) {
...
void ObjectKankyo_Fairies(ObjectKankyo* this, PlayState* play) {
    ...
    if (play->envCtx.precipitation[PRECIP_SNOW_MAX] < 64 &&
        (gOotSave.entranceIndex != ENTR_KOKIRI_FOREST_0 || gSaveContext.sceneLayer != 4 ||
