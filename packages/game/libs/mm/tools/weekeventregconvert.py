...
def updateCheck(fileContents: str) -> str:
    # gMmSave.saveInfo.weekEventReg[86] & 2
    checkRegex = re.compile(rf"gMmSave.saveInfo.weekEventReg\[(?P<index>{NUMBER_PATTERN})\]\s*\&\s*(?P<mask>{NUMBER_PATTERN})")

    return applyChange(fileContents, checkRegex, getCheckMacro)

def updateSet(fileContents: str) -> str:
    # gMmSave.saveInfo.weekEventReg[51] |= 0x10
    setRegex = re.compile(rf"gMmSave.saveInfo.weekEventReg\[(?P<index>{NUMBER_PATTERN})\]\s*\|=\s*(?P<mask>{NUMBER_PATTERN})")

    return applyChange(fileContents, setRegex, getSetMacro)

def updateClear(fileContents: str) -> str:
    # gMmSave.saveInfo.weekEventReg[85] &= (u8)~0x80
    clearRegex = re.compile(rf"gMmSave.saveInfo.weekEventReg\[(?P<index>{NUMBER_PATTERN})\]\s*\&=\s*(\(u8\))?~(?P<mask>{NUMBER_PATTERN})")
