    ...
    if (Message_ShouldAdvance(play) && (this->unk_288 == 0x2716)) {
        switch (play->msgCtx.choiceIndex) {
            case 0:
                ...
                } else if (gMmSave.saveInfo.playerData.rupees < play->msgCtx.unk1206C) {
                    Audio_PlaySfx(NA_SE_SY_ERROR);
            ...
            case 1:
                if (GET_CUR_EQUIP_VALUE(EQUIP_TYPE_SWORD) == EQUIP_VALUE_SWORD_NONE) {
                    ...
                    this->unk_288 = 0x272C;
                    Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, ENKENDOJS_ANIM_2);
                } else if (gMmSave.saveInfo.playerData.rupees < play->msgCtx.unk12070) {
                    Audio_PlaySfx(NA_SE_SY_ERROR);
                    Message_StartTextbox(play, 0x2718, &this->actor);
                    this->unk_288 = 0x2718;
                } else {
...
void func_80B276D8(EnKendoJs* this, PlayState* play) {
    if (Actor_HasParent(&this->actor, play)) {
        ...
        Actor_OfferGetItem(&this->actor, play, GI_MM_HEART_PIECE, 800.0f, 100.0f);
    } else {
        Actor_OfferGetItem(&this->actor, play, GI_MM_RUPEE_RED, 800.0f, 100.0f);
    }
}
