...
#define FLAGS (ACTOR_FLAG_MM_UPDATE_CULLING_DISABLED | ACTOR_FLAG_MM_20)
...
Gfx gObjGrass_D_809AA9F0[] = {
    gsDPPipeSync(),
    gsDPSetTextureLUT(G_TT_NONE),
    gsSPTexture(0xFFFF, 0xFFFF, 0, G_TX_RENDERTILE, G_ON),
    gsDPLoadTextureBlock(gameplay_field_keep_Tex_007010, G_IM_FMT_RGBA, G_IM_SIZ_16b, 32, 32, 0, G_TX_NOMIRROR |
...
OVL_INFO_ACTOR(ACTOR_OBJ_GRASS, Obj_Grass_Profile);
    ...
    for (i = 0; i < this->activeGrassGroups; i++) {
        ...
        if (grassGroup->flags & OBJ_GRASS_GROUP_DRAW) {
            ...
            for (j = 0; j < grassGroup->count; j++) {
                ...
                if ((grassElem->flags & OBJ_GRASS_ELEM_DRAW) && grassElem->alpha > 0) {
                    ...
                    ObjGrass_PreDraw(grassElem, play);
