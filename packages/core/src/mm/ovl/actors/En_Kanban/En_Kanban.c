...
static void EnKanban_LocText(PlayState* play)
{
    ...
    play->msgCtx.font.textBuffer.schar[0] = 0x01; /* Wooden texture */
    start = b;
...
void EnKanban_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params != ENKANBAN_PIECE) {
        ...
        if (this->actor.params == ENKANBAN_FISHING) {
            ...
            this->actor.messageId = this->actor.params | ENKANBAN_FISHING;
            if (this->actor.params == ENKANBAN_LOCATION_NAME)
                this->actor.messageId = 0x303;
        }

        this->bounceX = 1;
        this->partFlags = 0xFFFF;
        Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 50.0f, UPDBGCHECKINFO_FLAG_MM_4);
        func_80954960(this);
    }
}
    ...
    switch (this->actionState) {
            ...
            if ((this->invincibilityTimer == 0) && (this->collider.base.acFlags & AC_HIT)) {
                ...
                if (this->unk_19C != this->collider.base.ac) {
                    ...
                    if (piece != NULL) {
                        ...
                        piece->airTimer = 100;
                        piece->actor.flags &= ~ACTOR_FLAG_MM_ATTENTION_ENABLED;
                        piece->actor.flags |= ACTOR_FLAG_MM_UPDATE_DURING_OCARINA;
                        this->cutMarkTimer = 5;
            ...
            switch (this->ocarinaFlag) {
                ...
                case 1:
                    if ((play->msgCtx.ocarinaMode == OCARINA_MODE_END) &&
                        (play->msgCtx.ocarinaSong == OCARINA_SONG_HEALING)) {
        ...
        case ENKANBAN_REPAIR: {
            ...
            if (((distX + distY + distZ) == 0.0f) &&
                ...
                signpost->actor.flags |= ACTOR_FLAG_MM_ATTENTION_ENABLED;
                Actor_Kill(&this->actor);
                return;
            }
            break;
...
void EnKanban_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionState != ENKANBAN_SIGN) {
        ...
        if ((player->transformation == MM_PLAYER_FORM_HUMAN) || (player->transformation == MM_PLAYER_FORM_DEKU)) {
            phi_f0 = -15.0f;
        }
    ...
    if ((this->actor.projectedPos.z <= 400.0f) && (this->actor.projectedPos.z > 0.0f) &&
        ...
        if (dayTime >= CLOCK_TIME(12, 0)) {
            dayTime = (CLOCK_TIME(24, 0) - 1) - dayTime;
        }
