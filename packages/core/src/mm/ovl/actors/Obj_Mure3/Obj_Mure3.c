...
void func_8098F040(Actor_ObjMure3* this, PlayState* play) {
    ...
    for (i = 0; i < 5; i++, spawnPos.y += 20.0f) {
        if (!((this->unk164 >> i) & 1)) {
            this->unk148[i] = (Actor_EnItem00*)Item_DropCollectible2(play, &spawnPos, 0x10001);
...
void func_8098F110(Actor_ObjMure3* this, PlayState* play) {
    ...
    for (i = 0, dist = -40.0f; i < 5; i++, dist += 20.0f) {
        if (!((this->unk164 >> i) & 1)) {
            ...
            this->unk148[i] = (Actor_EnItem00*)Item_DropCollectible2(play, &spawnPos, 0x4000);
            if (this->unk148[i] != NULL) {
                this->unk148[i]->actor.room = this->actor.room;
            }
        }
...
void func_8098F220(Actor_ObjMure3* this, PlayState* play) {
    ...
    if (!((this->unk164 >> 6) & 1)) {
        ...
        this->unk160 = (Actor*)Item_DropCollectible2(play, &spawnPos, 0x4002);
        if (this->unk160 != NULL) {
...
void func_8098F438(Actor_ObjMure3* this, PlayState* play) {
    ...
    for (i = 0; i < count; i++) {
        Actor_EnItem00** collectible = &this->unk148[i];
