#include "En_Wood02.h"
#include <assets/mm/objects/object_wood02.h>
...
OVL_INFO_ACTOR(ACTOR_EN_WOOD02, En_Wood02_Profile);
...
void EnWood02_Update(Actor* thisx, PlayState* play2) {
    ...
    if ((thisx->params < WOOD_BUSH_GREEN_SMALL) || (thisx->params == WOOD_TREE_SPECIAL)) {
        ...
        if ((this->unk_146 >= -1) &&
            (((player->rideActor == NULL) && (sqrtf(thisx->xyzDistToPlayerSq) < 20.0f) && (player->linearVelocity != 0.0f)) ||
