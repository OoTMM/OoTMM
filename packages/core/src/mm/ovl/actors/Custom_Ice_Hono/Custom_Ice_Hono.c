...
void EnIceHono_CapturableFlame(EnIceHono* this, PlayState* play) {
    ...
    Actor_PlaySfx_FlaggedCentered1(&this->actor, NA_SE_EV_FIRE_PILLAR_S - SFX_FLAG);
}
*/
...
void EnIceHono_DropFlame(EnIceHono* this, PlayState* play) {
    ...
    if (bgFlag != 0) {
        s32 i;
        for (i = 0; i < 8; i++) {
            Actor_Spawn(&play->actorCtx, play, ACTOR_CUSTOM_ICE_HONO, this->actor.world.pos.x, this->actor.world.pos.y,
...
void EnIceHono_SpreadFlames(EnIceHono* this, PlayState* play) {
    ...
    this->actor.scale.z = this->actor.scale.x;
    Actor_MoveXZGravity(&this->actor);
    Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, this->actor.scale.x * 3500.0f, 0.0f, UPDBGCHECKINFO_FLAG_OOT_2);
    if (this->timer < 25) {
        this->alpha -= 10;
...
void EnIceHono_SmallFlameMove(EnIceHono* this, PlayState* play) {
    ...
    Actor_MoveXZGravity(&this->actor);
    Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 0.0f, UPDBGCHECKINFO_FLAG_OOT_0 | UPDBGCHECKINFO_FLAG_OOT_2);
...
void EnIceHono_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params == 0) {
        Actor_PlaySfx_FlaggedCentered1(&this->actor, NA_SE_IT_FLAME - SFX_FLAG);
    }
    if ((this->actor.params == -1) || (this->actor.params == 0)) {
        this->unk_154 += 0x1111;
        this->unk_156 += 0x4000;
...
void EnIceHono_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPSegment(POLY_XLU_DISP++, 0x08,
               Gfx_TwoTexScroll(play->state.gfxCtx, G_TX_RENDERTILE, 0, 0, 32, 64, 1, 0,
                                (play->state.frameCount * -20) % 512, 32, 128));
    ...
    gSPDisplayList(POLY_XLU_DISP++, 0x0407D590); /* gEffFire1DL */

    CLOSE_DISPS();
}

ActorProfile En_Ice_Hono_Profile = {
    /**/ ACTOR_CUSTOM_ICE_HONO,
    /**/ ACTORCAT_ITEMACTION,
    /**/ FLAGS,
...
OVL_INFO_ACTOR(ACTOR_CUSTOM_ICE_HONO, En_Ice_Hono_Profile);
