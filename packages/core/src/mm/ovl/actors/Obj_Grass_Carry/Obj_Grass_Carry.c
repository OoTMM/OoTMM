...
#define FLAGS (ACTOR_FLAG_MM_UPDATE_CULLING_DISABLED | ACTOR_FLAG_MM_20 | ACTOR_FLAG_THROW_ONLY)
...
OVL_INFO_ACTOR(ACTOR_OBJ_GRASS_CARRY, Obj_Grass_Carry_Profile);
...
void ObjGrassCarry_DropCollectible(ObjGrassCarry* this, s16 dropTable, PlayState* play) {
    ...
    if ((dropTable & 0x10) == 0) {
        Item_DropCollectibleRandom(play, NULL, &this->actor.world.pos, dropTable * 0x10);
    }
}
...
void ObjGrassCarry_SpawnFragments(Vec3f* basePos, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(sUnitDirections); i++) {
        ...
        EffectSsKakera_Spawn(play, &pos, &velocity, &pos, -100, 64, 40, 3, 0,
                             sFragmentScales[(s32)(Rand_ZeroOne() * 111.1f) & 7], 0, 0, 80, -1, OBJECT_GAMEPLAY_KEEP,
                             gKakeraLeafMiddleDL);
...
void ObjGrassCarry_Main(ObjGrassCarry* this, PlayState* play) {
    ...
    if (Actor_HasParent(thisx, play)) {
        ...
        thisx->yDistanceFromLink = Actor_HeightDiff(&this->actor, &player->actor);
        thisx->xyzDistToPlayerSq = SQ(thisx->xzDistToPlayer) + SQ(thisx->yDistanceFromLink);
...
void ObjGrassCarry_Fall(ObjGrassCarry* this, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_GROUND_TOUCH | BGCHECKFLAG_WALL)) || atHit ||
        ...
        ObjGrassCarry_DropCollectible(this, this->dropTable, play);
...
void ObjGrassCarry_Draw(Actor* thisx, PlayState* play) {
    ObjGrassCarry* this = THIS;
    ObjGrassCarry_PreDraw(this->grassElem, play);
