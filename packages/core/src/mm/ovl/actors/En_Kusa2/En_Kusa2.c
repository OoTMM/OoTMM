#include <actors/En_Kusa2/En_Kusa2.h>
...
OVL_INFO_ACTOR(ACTOR_EN_KUSA2, En_Kusa2_Profile);
...
static EnKusa2* EnKusa2_SpawnBush(EnKusa2* this, PlayState* play, float dx, float dz) {
    EnKusa2* actor;
    actor = (EnKusa2*)Actor_SpawnAsChildAndCutscene(
            &play->actorCtx, play, ACTOR_EN_KUSA2, this->actor.world.pos.x + dx, this->actor.world.pos.y,
            this->actor.world.pos.z + dz, 0, Rand_Next() >> 0x10, 0, 1, this->actor.csId, this->actor.halfDaysBits, NULL);
    return actor;
}
...
void func_80A5B160(EnKusa2* this, PlayState* play) {
    ...
    if (this->unk_194[0] == NULL) {
        ptr = this->unk_194;
        actor = EnKusa2_SpawnBush(this, play, 0.0f, 0.0f);
        *ptr = actor;
    ...
    for (i = 1; i < ARRAY_COUNT(this->unk_194); i++) {
        ...
        if (this->unk_194[i] == NULL) {
            ptr = &this->unk_194[i];
            actor = EnKusa2_SpawnBush(this, play, Math_SinS(temp_s1) * 80.0f, Math_CosS(temp_s1) * 80.0f);
            *ptr = actor;
            if (*ptr != NULL) {
                (*ptr)->actor.room = this->actor.room;
                (*ptr)->unk_1C0 = this;
...
void func_80A5BD14(EnKusa2* this, PlayState* play, s32 arg2) {
    ...
    if (kusa2 != NULL) {
        ...
        kusa2->unk_1BC += arg2;
        EnKusa2_UpdateCurrentXflag((u8)kusa2->unk_1BC);
    }
}
...
void EnKusa2_Init(Actor* thisx, PlayState* play) {
    ...
    if (!ENKUSA2_GET_1(&this->actor)) {
        ...
        this->actor.flags |= ACTOR_FLAG_MM_20;
