...
static void CustomWarp_OnTrigger(Actor_CustomWarp* this, GameState_Play* play)
{
    ...
    case SWITCH_COAST_CLEAR:
        ...
        break;
    case SWITCH_CLOCK_TOWER:
        play->nextEntrance = ENTR_MM_CLOCK_TOWN_FROM_CLOCK_TOWER;
        break;
    }
}
#endif
...
void comboSpawnCustomWarps(GameState_Play* play)
{
    ...
    if (comboConfig(CFG_ER_MOON) && play->sceneId == SCE_MM_CLOCK_TOWER_ROOFTOP)
    {
        variable = SWITCH_CLOCK_TOWER;
        x = -212.f;
        y = 30.f;
        z = 2.f;
    }
