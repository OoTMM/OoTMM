...
#include "assets/oot/objects/object_fr.h"
...
// Animation Functions
void EnFr_SetupJumpingOutOfWater(Actor_EnFr* this, PlayState* play);
void EnFr_JumpingOutOfWater(Actor_EnFr* this, PlayState* play);
void EnFr_OrientOnLogSpot(Actor_EnFr* this, PlayState* play);
void EnFr_ChooseJumpFromLogSpot(Actor_EnFr* this, PlayState* play);
void EnFr_JumpingUp(Actor_EnFr* this, PlayState* play);
void EnFr_JumpingBackIntoWater(Actor_EnFr* this, PlayState* play);
void EnFr_DecrementBlinkTimerUpdate(Actor_EnFr* this);

// Activation
void EnFr_Idle(Actor_EnFr* this, PlayState* play);
void EnFr_Activate(Actor_EnFr* this, PlayState* play);
void EnFr_ActivateCheckFrogSong(Actor_EnFr* this, PlayState* play);

// Listening for Child Songs
void func_80A1BE98(Actor_EnFr* this, PlayState* play);
void EnFr_ListeningToOcarinaNotes(Actor_EnFr* this, PlayState* play);
void EnFr_ChildSong(Actor_EnFr* this, PlayState* play);
void EnFr_ChildSongFirstTime(Actor_EnFr* this, PlayState* play);

// Frog Song for HP Functions
void EnFr_TalkBeforeFrogSong(Actor_EnFr* this, PlayState* play);
void EnFr_SetupFrogSong(Actor_EnFr* this, PlayState* play);
void EnFr_ContinueFrogSong(Actor_EnFr* this, PlayState* play);
void EnFr_OcarinaMistake(Actor_EnFr* this, PlayState* play);

// Reward Functions
void EnFr_SetupReward(Actor_EnFr* this, PlayState* play, u8 unkCondition);
void EnFr_PrintTextBox(Actor_EnFr* this, PlayState* play);
void EnFr_TalkBeforeReward(Actor_EnFr* this, PlayState* play);
void EnFr_SetReward(Actor_EnFr* this, PlayState* play);

// Deactivate
void EnFr_Deactivate(Actor_EnFr* this, PlayState* play);
void EnFr_GiveReward(Actor_EnFr* this, PlayState* play);
void EnFr_SetIdle(Actor_EnFr* this, PlayState* play);
...
#define FROG_HAS_SONG_BEEN_PLAYED(frogSongIndex) \
    (gSaveContext.save.info.eventsChk[0xd] &   \
...
static u16 sFrogSongIndexToEventChkInfSongsForFrogsMask[] = {
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_ZL),     // FROG_ZL
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_EPONA),  // FROG_EPONA
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_SARIA),  // FROG_SARIA
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_SUNS),   // FROG_SUNS
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_SOT),    // FROG_SOT
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_STORMS), // FROG_STORMS
    EVENTCHKINF_MASK(EV_OOT_CHK_FROGS_GAME),   // FROG_CHOIR_SONG
    0,                                         // FROG_NO_SONG
};
...
void EnFr_OrientUnderwater(Actor_EnFr* this) {
    Vec3f vec1;
    Vec3f vec2;
...
void EnFr_Init(Actor* thisx, PlayState* play) {
    Actor_EnFr* this = (Actor_EnFr*)thisx;

    if (this->actor.params == 0) {
        this->actor.fini = NULL;
...
void EnFr_DrawIdle(Actor_EnFr* this) {
    this->actor.draw = (this->actor.params - 1) != FROG_PURPLE ? NULL : EnFr_Draw;
}

void EnFr_DrawActive(Actor_EnFr* this) {
...
void EnFr_Update(Actor* thisx, PlayState* play) {
    ...
    if (Object_IsLoaded(&play->objectCtx, this->requiredObjectSlot)) {
        ...
        // frog
        SkelAnime_InitFlex(play, &this->skelAnime, (void*)object_fr_Skel_00B498, (void*)object_fr_Anim_001534, this->jointTable,
                           this->morphTable, 24);
        // butterfly
        SkelAnime_Init(play, &this->skelAnimeButterfly, (void*)gButterflySkel, (void*)gButterflyAnim, this->jointTableButterfly,
...
void EnFr_IsDivingIntoWater(Actor_EnFr* this, PlayState* play) {
    WaterBox* waterBox;
    f32 waterSurface;
...
void EnFr_DivingIntoWater(Actor_EnFr* this, PlayState* play) {
    Vec3f vec;
...
s32 EnFr_IsBelowLogSpot(Actor_EnFr* this, f32* yDistToLogSpot) {
...
int EnFr_IsAboveAndWithin30DistXZ(Player* player, Actor_EnFr* this) {
...
void EnFr_DecrementBlinkTimer(Actor_EnFr* this) {
...
void EnFr_SetupJumpingOutOfWater(Actor_EnFr* this, PlayState* play) {
    if (sEnFrPointers.flags == sTimerJumpingOutOfWater[this->actor.params - 1]) {
        Animation_Change(&this->skelAnime, (void*)object_fr_Anim_0007BC, 1.0f, 0.0f,
                         Animation_GetLastFrame((void*)object_fr_Anim_0007BC), ANIMMODE_ONCE, 0.0f);
...
void EnFr_OrientOnLogSpot(Actor_EnFr* this, PlayState* play) {
    ...
    if ((rotYRemaining == 0) && (this->skelAnime.curFrame == this->skelAnime.endFrame)) {
        ...
        Animation_Change(&this->skelAnime, (void*)object_fr_Anim_001534, 1.0f, 0.0f,
                         Animation_GetLastFrame((void*)object_fr_Anim_001534), ANIMMODE_LOOP, 0.0f);
    }
}
...
void EnFr_JumpingUp(Actor_EnFr* this, PlayState* play) {
    ...
    if (EnFr_IsBelowLogSpot(this, &yDistToLogSpot)) {
        this->isJumpingUp = false;
        this->actor.gravity = 0.0f;
        Animation_Change(&this->skelAnime, (void*)object_fr_Anim_0011C0, 1.0f, 0.0f,
                         Animation_GetLastFrame((void*)object_fr_Anim_0011C0), ANIMMODE_LOOP, 0.0f);
        this->actionFunc = EnFr_ChooseJumpFromLogSpot;
...
void EnFr_SetScaleActive(Actor_EnFr* this, PlayState* play) {
...
void EnFr_ButterflyPath(Actor_EnFr* this, PlayState* play) {
...
s32 EnFr_SetupJumpingUp(Actor_EnFr* this, s32 frogIndex) {
    Actor_EnFr* frog = sEnFrPointers.frogs[frogIndex];
    u8 semitone;
...
void EnFr_ActivateCheckFrogSong(Actor_EnFr* this, PlayState* play) {
    if (sEnFrPointers.flags == 11) {
        ...
        if (GET_EVENTCHKINF(EV_OOT_CHK_FROGS_ZL) && GET_EVENTCHKINF(EV_OOT_CHK_FROGS_EPONA) &&
            GET_EVENTCHKINF(EV_OOT_CHK_FROGS_SARIA) && GET_EVENTCHKINF(EV_OOT_CHK_FROGS_SUNS) &&
            GET_EVENTCHKINF(EV_OOT_CHK_FROGS_SOT) && GET_EVENTCHKINF(EV_OOT_CHK_FROGS_STORMS)) {
...
void EnFr_ChildSong(Actor_EnFr* this, PlayState* play) {
    if (this->jumpCounter < 48) {
        ...
        if (this->songIndex == FROG_STORMS) {
            this->actor.messageId = 0x40AA;
            EnFr_SetupReward(this, play, false);
        } else if (!FROG_HAS_SONG_BEEN_PLAYED(this->songIndex)) {
            Actor_EnFr* frog = sEnFrPointers.frogs[sSongToFrog[this->songIndex]];
...
u8 EnFr_GetNextNoteFrogSong(u8 ocarinaNoteIndex) {
    if (!GET_EVENTCHKINF(EV_OOT_CHK_FROGS_GAME)) {
...
s32 EnFr_IsFrogSongComplete(Actor_EnFr* this, PlayState* play) {
    u8 index;
    u8 ocarinaNote;
    ...
    if (this->frogSongTimer == 0) {
        ...
        if (play->msgCtx.msgMode == MSGMODE_FROGS_PLAYING) {
            ...
            for (i = 0; i < ARRAY_COUNT(sEnFrPointers.frogs); i++) {
                Actor_EnFr* frog = sEnFrPointers.frogs[i];
...
void EnFr_PrintTextBox(Actor_EnFr* this, PlayState* play) {
    Message_StartTextbox(play, this->actor.messageId, &this->actor);
...
void EnFr_ComboGiveItem(Actor_EnFr* this, PlayState* play) {
    int npc;

    npc = -1;
    if (this->reward == GI_OOT_HEART_PIECE)
    {
    ...
    if (this->reward == GI_OOT_RUPEE_PURPLE) {
        if(this->songIndex == FROG_ZL && GetEventChk(EV_OOT_CHK_FROGS_ZL)) {
            npc = NPC_OOT_FROGS_ZL;
        }
        else if(this->songIndex == FROG_EPONA && GetEventChk(EV_OOT_CHK_FROGS_EPONA)) {
            npc = NPC_OOT_FROGS_EPONA;
        }
        else if(this->songIndex == FROG_SUNS && GetEventChk(EV_OOT_CHK_FROGS_SUNS)) {
            npc = NPC_OOT_FROGS_SUNS;
        }
        else if(this->songIndex == FROG_SARIA && GetEventChk(EV_OOT_CHK_FROGS_SARIA)) {
            npc = NPC_OOT_FROGS_SARIA;
        }
        else if(this->songIndex == FROG_SOT && GetEventChk(EV_OOT_CHK_FROGS_SOT)) {
            npc = NPC_OOT_FROGS_SOT;
        }
    }

    comboGiveItemNpc(&this->actor, play, (s16)this->reward, npc, 30.f, 100.f);
}
...
void EnFr_Deactivate(Actor_EnFr* this, PlayState* play) {
    ...
    for (frogIndex = 0; frogIndex < ARRAY_COUNT(sEnFrPointers.frogs); frogIndex++) {
        ...
        if (frog == NULL || !frog->isDeactivating) {
            return;
        }
    }
...
OVL_INFO_ACTOR(ACTOR_EN_FR, En_Fr_Profile);
