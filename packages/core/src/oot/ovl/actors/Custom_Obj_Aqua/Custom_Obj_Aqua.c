...
static InitChainEntry sInitChain[] = {
    ...
    ICHAIN_F32(cullingVolumeDistance, 4000, ICHAIN_CONTINUE), ICHAIN_F32(cullingVolumeScale, 300, ICHAIN_CONTINUE),
...
void func_80ACBA10(ObjAqua* this) {
    ...
    func_80038A28(this->actor.floorPoly, this->actor.world.pos.x, this->actor.floorHeight, this->actor.world.pos.z,
                  &sp2C);
    Matrix_MtxFToYXZRotS(&sp2C, &this->actor.shape.rot, false);
}
...
void func_80ACBC8C(ObjAqua* this, PlayState* play) {
    if (this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_WATER)) {
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            ...
            func_80ACBA10(this);
            Actor_PlaySfx(&this->actor, NA_SE_EV_BOMB_DROP_WATER); /* TODO change to NA_SE_EV_BOTTLE_WATERING */
            func_80ACBD34(this);
        } else {
            func_80ACB6A0(this, play);
            SfxSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 0x28, NA_SE_EV_BOMB_DROP_WATER);
...
void ObjAqua_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.update != NULL) {
        ...
        Actor_UpdateBgCheckInfo(play, &this->actor, 12.0f, 4.0f, 0.0f, UPDBGCHECKINFO_FLAG_WALL | UPDBGCHECKINFO_FLAG_FLOOR);
...
void ObjAqua_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, 0x04052a10); /* gEffFire1DL */

    CLOSE_DISPS();
}

ActorProfile Obj_Aqua_InitVars = {
    /**/ ACTOR_CUSTOM_OBJ_AQUA,
    /**/ ACTORCAT_ITEMACTION,
    /**/ FLAGS,
...
OVL_INFO_ACTOR(ACTOR_CUSTOM_OBJ_AQUA, Obj_Aqua_InitVars);
