...
OVL_INFO_ACTOR(ACTOR_OBJ_BEAN, Obj_Bean_Profile);
...
s32 ObjBean_CheckForHorseTrample(ObjBean* this, PlayState* play) {
    Actor* currentActor = play->actorCtx.actors[ACTORCAT_BG].first;
...
void ObjBean_Init(Actor* thisx, PlayState* play) {
    ...
    if (gOotSave.age == AGE_ADULT) {
        if (Flags_GetSwitch(play, PARAMS_GET_U(this->dyna.actor.params, 0, 6))) {
            ...
            ObjBean_InitDynaPoly(this, play, (void*)&gMagicBeanPlatformCol, DYNA_TRANSFORM_POS | DYNA_TRANSFORM_ROT_Y);
...
void ObjBean_SetupWaitForBean(ObjBean* this) {
    ...
    this->dyna.actor.messageId = 0x2F;
}
